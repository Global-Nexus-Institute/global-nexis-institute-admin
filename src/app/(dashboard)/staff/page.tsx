...
import UserTable from "../users/_components/user-table/UserTable";

const UsersPage: React.FC = () => {
  const [users, setUsers] = useState<UsersDataType[]>([]);
  const [refresh, setReferesh] = useState(false);

  const { data: staff, loading } = useAppSelector(
    (state: RootState) => state.users,
  );

  const { getStaff } = useUsers();
  ...
  return (
    <div>
      ...
      <Row className=" h-50 my-2 bodrder p-5 w-full gap-4" gutter={6}>
        <Col span={6}>
          <Card>
            <Statistic title="Total Staffs" value={staff.length} />
          </Card>
        </Col>
        <Col span={6}>
          {" "}
